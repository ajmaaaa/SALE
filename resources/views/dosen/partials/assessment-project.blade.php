{{-- ========================================================= --}}
{{-- PARTIAL: PENILAIAN PROJECT (PRJ-01)                       --}}
{{-- ========================================================= --}}
@php
    $projectStudents = [
    ...
    <div class="assessment-overview-layout flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div class="space-y-2 flex-1">
            <div class="flex items-center gap-2">
                <span class="status font-bold text-brand bg-brand-soft border-brand-soft uppercase text-[11px]">PROYEK KELOMPOK</span>
                <span class="status font-semibold text-muted bg-canvas text-[11px]">Bobot: 25%</span>
            </div>
            <h2 class="text-xl font-bold text-ink">Proyek Pengembangan Aplikasi Web</h2>
            <p class="text-xs text-muted leading-relaxed max-w-2xl">
                Proyek kolaboratif tim merancang antarmuka pengguna berbasis UI/UX modern, arsitektur basis data relasional terintegrasi, serta pembuatan API RESTful dan pengujian kinerja sistem.
            </p>
            <div class="assessment-meta flex flex-wrap items-center gap-4 pt-1 text-xs text-muted">
                <span class="flex items-center gap-1.5">
                    ...
                    Format: <strong class="text-ink font-semibold">Implementasi Kode &amp; Presentasi Tim</strong>
                </span>
                <span class="flex items-center gap-1.5">
                    ...
                    CPMK Terukur: <strong class="text-ink font-semibold">CPMK 02, CPMK 03</strong>
                </span>
            </div>
        </div>
        ...
        <div class="assessment-targets flex flex-wrap items-center gap-3 shrink-0">
            <div class="assessment-target rounded-lg p-3 text-center min-w-[120px]">
                ...
                <p class="text-xs font-semibold text-ink mt-0.5">Skor ≥ 70</p>
            </div>
...
<div class="surface assessment-toolbar p-4 flex flex-wrap items-center justify-between gap-4">
    ...
    <div class="assessment-toolbar-actions flex items-center gap-2.5 shrink-0">
        ...
        <button type="button" onclick="triggerOBESync('Data nilai Project diekspor')" class="button-secondary text-xs">
    ...
    <table class="admin-table assessment-table w-full" id="students-table">
        <thead>
            <tr>
                ...
                <th scope="col" class="text-center min-w-[130px]">NILAI PROJECT <span class="block font-normal mt-1">(BOBOT 25%)</span></th>
                <th scope="col" class="text-center min-w-[140px]">CAPAIAN CPMK-01 <span class="block font-normal mt-1">(≥70)</span></th>
        ...
        <tbody>
            @foreach($projectStudents as $stu)
                <tr class="student-row" data-name="{{ strtolower($stu['name']) }}" data-number="{{ $stu['number'] }}" data-status="{{ $stu['status_key'] }}">
                    ...
                    {{-- Nilai Project: READONLY, dihitung otomatis dari rata-rata CPMK --}}
                    ...
                    <td class="text-center align-middle">
                        <input type="number" min="0" max="100" step="0.5"
                               ...
                               class="field text-center font-mono text-xs font-bold py-1 px-1.5 w-16 mx-auto block border border-line bg-white shadow-none
                                      {{ $stu['cpmk1'] >= 70 ? 'text-emerald-700' : 'text-danger' }}"
...
<div class="assessment-count flex flex-wrap items-center justify-between gap-4">
    <p class="text-xs text-muted" id="student-count-text">
        Menampilkan <strong class="text-ink">{{ count($projectStudents) }}</strong> dari <strong class="text-ink">{{ count($projectStudents) }}</strong> mahasiswa terdaftar
    </p>
...
<div class="surface assessment-savebar p-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div class="flex items-center gap-3 text-xs text-muted">
        ...
        <div>
            <p class="font-bold text-ink">Kalkulasi Otomatis OBE (Project)</p>
            <p class="text-muted">Nilai Project berkontribusi 25% pada kalkulasi akhir CPMK 02 dan CPMK 03.</p>
        </div>
    </div>
    ...
    <div class="assessment-save-actions flex items-center gap-3 self-end sm:self-auto">
        ...
        <button type="button" onclick="triggerOBESync('Nilai Proyek berhasil disimpan!')" class="button-primary text-xs">
...
{{-- Script: Hitung ulang Nilai Project dari rata-rata CPMK --}}
<script>
function recalcScore(inputEl) {
    const studentId = inputEl.dataset.student;
    const row       = inputEl.closest('tr');

    // Ambil semua input CPMK di baris yang sama
    ...
    // Update tampilan nilai project (readonly)
    const scoreEl = document.getElementById('score-' + studentId);
    if (scoreEl) scoreEl.textContent = rounded;

    // Update warna input berdasarkan threshold
    cpmkInputs.forEach(inp => {
        const threshold = inp.dataset.cpmk === 'cpmk1' ? 70 : 80;
