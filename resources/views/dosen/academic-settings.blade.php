...
<button type="button" data-remove-row class="mt-7 text-muted" aria-label="Hapus baris">×</button>
@if($group==='cpmk')<label class="sm:col-span-2"><span class="form-label text-xs">Batas ketercapaian (0-100)</span><input class="field" required type="number" min="0" max="100" step="0.01" name="cpmk[{{ $index }}][threshold]" value="{{ $row['threshold'] ?? 65 }}"><span class="mt-1 block text-xs text-muted">Setiap CPMK wajib memenuhi batasnya sendiri.</span></label>@endif
</div>@endforeach</div>@if($group==='components')<p class="mt-4 text-sm font-semibold" data-weight-total>Total bobot: {{ array_sum(array_column($config['components'],'weight')) }}%</p><p class="mt-1 text-xs text-muted">Total harus 100%. Nilai kosong berarti belum dinilai, bukan nol.</p>@endif</section>@endforeach
