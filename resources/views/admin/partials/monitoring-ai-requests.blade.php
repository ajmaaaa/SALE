@php($aiRequestRows = $aiRequestRows ?? collect())
<div class="mt-6" aria-labelledby="ai-requests-heading">
    <div class="flex flex-wrap items-center justify-between gap-2">
        ...
        <span class="text-xs text-muted">{{ $demo ? '12 respons contoh' : ($aiRequestRows->isEmpty() ? 'Belum ada respons tercatat' : $aiRequestRows->count().' panggilan terbaru') }}</span>
    </div>
    ...
    <div tabindex="0" role="region" aria-label="Riwayat token per request AI" class="mt-4 max-h-96 overflow-auto rounded-xl border border-line/60 focus-visible:outline-2 focus-visible:outline-brand">
        <table class="admin-table w-full">
            ...
            <thead class="sticky top-0 z-10 bg-canvas"><tr><th>Model / tahap</th><th class="whitespace-nowrap">Token input</th><th class="whitespace-nowrap">Token output</th><th class="whitespace-nowrap">Thinking</th><th class="whitespace-nowrap">Est. USD</th><th class="whitespace-nowrap">Latensi</th><th>Waktu selesai</th></tr></thead>
            <tbody>
                @if($demo)
                    ...
                    @endforeach
                @elseif($aiRequestRows->isNotEmpty())
                    ...
                    @endforeach
                @else
                    <tr><td colspan="7" class="px-4 py-10 text-center"><p class="text-sm font-semibold text-ink">Belum ada riwayat request</p><p class="mx-auto mt-2 max-w-sm text-xs leading-5 text-muted">Setelah API AI aktif dan dipanggil, pemakaian aktual akan tampil di sini.</p></td></tr>
                @endif
            </tbody>
        </table>
    </div>
