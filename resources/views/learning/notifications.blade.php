...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-slate-200 pb-px">
        <nav class="flex gap-2 sm:gap-6 text-xs sm:text-sm font-semibold overflow-x-auto" aria-label="Kategori Notifikasi">
            {{-- Semua --}}
            <a href="{{ route('mahasiswa.notifications') }}"
               class="pb-3 border-b-2 -mb-px transition flex items-center gap-2 whitespace-nowrap {{ empty($selectedCategory) ? 'border-[#102f50] text-[#102f50] font-bold' : 'border-transparent text-slate-500 hover:text-slate-800' }}">
                <span>Semua</span>
                <span class="rounded-full px-2 py-0.5 text-xs font-bold {{ empty($selectedCategory) ? 'bg-[#102f50] text-white' : 'bg-slate-100 text-slate-700' }}">
            ...
            <a href="{{ route('mahasiswa.notifications', ['category' => 'tugas']) }}"
               class="pb-3 border-b-2 -mb-px transition flex items-center gap-2 whitespace-nowrap {{ $selectedCategory === 'tugas' ? 'border-[#102f50] text-[#102f50] font-bold' : 'border-transparent text-slate-500 hover:text-slate-800' }}">
                <span>Tugas &amp; Kuis</span>
                @if(isset($categoryCounts['tugas']))
                    <span class="rounded-full px-2 py-0.5 text-xs font-bold {{ $selectedCategory === 'tugas' ? 'bg-[#102f50] text-white' : 'bg-slate-100 text-slate-700' }}">
            ...
            <a href="{{ route('mahasiswa.notifications', ['category' => 'nilai']) }}"
               class="pb-3 border-b-2 -mb-px transition flex items-center gap-2 whitespace-nowrap {{ $selectedCategory === 'nilai' ? 'border-[#102f50] text-[#102f50] font-bold' : 'border-transparent text-slate-500 hover:text-slate-800' }}">
                <span>Nilai</span>
                @if(isset($categoryCounts['nilai']))
                    <span class="rounded-full px-2 py-0.5 text-xs font-bold {{ $selectedCategory === 'nilai' ? 'bg-[#102f50] text-white' : 'bg-slate-100 text-slate-700' }}">
            ...
            <a href="{{ route('mahasiswa.notifications', ['category' => 'sistem']) }}"
               class="pb-3 border-b-2 -mb-px transition flex items-center gap-2 whitespace-nowrap {{ $selectedCategory === 'sistem' ? 'border-[#102f50] text-[#102f50] font-bold' : 'border-transparent text-slate-500 hover:text-slate-800' }}">
                <span>Sistem</span>
                @if(isset($categoryCounts['sistem']))
                    <span class="rounded-full px-2 py-0.5 text-xs font-bold {{ $selectedCategory === 'sistem' ? 'bg-[#102f50] text-white' : 'bg-slate-100 text-slate-700' }}">
            ...
            <a href="{{ route('mahasiswa.notifications', ['category' => 'diskusi']) }}"
               class="pb-3 border-b-2 -mb-px transition flex items-center gap-2 whitespace-nowrap {{ $selectedCategory === 'diskusi' ? 'border-[#102f50] text-[#102f50] font-bold' : 'border-transparent text-slate-500 hover:text-slate-800' }}">
                <span>Diskusi</span>
                @if(isset($categoryCounts['diskusi']))
                    <span class="rounded-full px-2 py-0.5 text-xs font-bold {{ $selectedCategory === 'diskusi' ? 'bg-[#102f50] text-white' : 'bg-slate-100 text-slate-700' }}">
            ...
            <div class="flex items-center gap-2 pb-2.5 sm:pb-3 shrink-0">
                ...
                @if(count($notifications) > 0)
                    <form action="{{ route('mahasiswa.notifications.clear') }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus semua notifikasi? Tindakan ini tidak dapat dibatalkan.');">
                        ...
                        @endif
                        <button type="submit" class="inline-flex items-center gap-1.5 rounded-lg bg-red-600 px-3 py-1.5 text-xs font-bold text-white shadow-2xs hover:bg-red-700 active:scale-[0.98] transition-all cursor-pointer">
                            <svg class="h-3.5 w-3.5 text-red-100" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
