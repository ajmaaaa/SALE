...
<div class="space-y-7">
    ...
    <div class="course-layout-grid grid items-start gap-7">
                            ...
                            <div class="divide-y divide-line/40">
                                @foreach($contents as $item)
                                    @php
                                        ...
                                        $itemUrl = $isCoding ? route('mahasiswa.assignment.code', $item['id']) : ($isDosen ? route('dosen.course.item', [$course['id'], $item['id']]) : route('mahasiswa.course.item', [$course['id'], $item['id']]));
                                    @endphp
        ...
        {{-- KOLOM KANAN (SIDEBAR DISAMPING): Forum Diskusi Kelas Real-Time --}}
        <aside class="space-y-6">
            @php
                ...
                if ($hasChatTables && $chatRoom && \App\Models\Message::where('room_id', $chatRoom->id)->exists()) {
                    $rawMessages = \App\Models\Message::where('room_id', $chatRoom->id)
                        ...
                        ->limit(35)
                        ->get();
                    $initialMessages = $rawMessages->reverse()->values()->map(fn($m) => $m->toChatPayload($chatUser));
                    ...
                    $initialMessages = $previewMessages->map(function ($m, $idx) use ($chatUser) {
                        return [
                            'id' => $idx + 1,
                            'room_id' => 1,
                            'content' => $m['message'],
                            ...
                            'date_label' => 'Hari ini',
                            'timestamp' => $m['timestamp'] ?? now()->timestamp,
                            'reply_to' => null,
                            'mentions' => [],
                        ];
                    });
            ...
            <section id="diskusi-kelas" aria-labelledby="discuss-heading" class="surface scroll-mt-24 p-5 rounded-xl border border-line/60 space-y-3">
                <div class="flex items-center justify-between border-b border-line/50 pb-3">
                    <div class="flex items-center gap-2">
                        <span class="inline-flex h-2.5 w-2.5 rounded-full bg-emerald-500 animate-pulse" title="Sistem Diskusi Terhubung"></span>
                    ...
                    <span id="chat-total-badge" class="text-[11px] font-medium text-muted">
                        {{ $initialMessages->count() }} pesan
                    </span>
                </div>

                {{-- Banner Pesan yang Disematkan Dosen (Bagian 10.4) --}}
                <div id="pinned-announcements-container" class="{{ $pinnedMessages->isEmpty() ? 'hidden' : '' }} rounded-xl border border-amber-200/80 bg-gradient-to-r from-amber-50/90 to-orange-50/70 p-3 text-xs shadow-2xs">
                    ...
                    <div id="pinned-messages-list" class="space-y-1.5 max-h-28 overflow-y-auto pr-1">
                ...
                {{-- Messages Stream Container (Bagian 4.2 & 6) --}}
                <div id="chat-messages" class="space-y-2.5 max-h-[400px] overflow-y-auto pr-1 flex flex-col scroll-smooth">
                    @forelse($initialMessages as $msg)
                        ...
                        @endphp
                        <div id="msg-bubble-{{ $msg['id'] }}" class="chat-message-row group flex w-full {{ $isMe ? 'justify-end' : 'justify-start' }}" data-message-id="{{ $msg['id'] }}" data-author="{{ $msg['author'] }}" data-date="{{ $msg['date_key'] }}">
                            <article class="min-w-0 w-fit max-w-[88%] rounded-xl border p-3 shadow-2xs transition-all {{ $isMe ? 'bg-[#edf4fb] border-[#cfe0f2]' : 'bg-canvas/70 border-line/60' }} relative hover:shadow-xs">
                                {{-- Header Pesan: Avatar, Nama, Badge Dosen / Pin --}}
                                <div class="flex items-center justify-between gap-2">
                                    <div class="flex items-center gap-2 min-w-0">
                                        ...
                                        <div class="min-w-0 flex items-center gap-1.5 flex-wrap">
                                            <span class="text-xs font-bold text-ink truncate">{{ $msg['author'] }}</span>
                                            @if($isMe)
                                            ...
                                            @endif
                                            @if($isPinned)
                                                <span id="pin-badge-{{ $msg['id'] }}" class="inline-flex items-center gap-0.5 text-[9px] font-bold text-amber-700 bg-amber-100/80 px-1 rounded border border-amber-200">
                                                    📌 Disematkan
                                                </span>
                                            @endif
                                        </div>
                                ...
                                {{-- Kutipan Balasan (Bagian 9: Reply Quote Bubble ala WhatsApp) --}}
                                ...
                                {{-- Bar Aksi Cepat (Reply, Pin, Hapus) --}}
                    ...
                    @empty
                        <div id="chat-empty-state" class="rounded-xl border border-line/60 bg-canvas p-6 text-center text-xs text-muted">
                            Belum ada pesan di forum kelas ini. Mulai diskusi dengan menulis pesan di bawah.
                        </div>
                    @endforelse
                </div>

                {{-- Area Input Pesan dengan Preview Reply & Dropdown Mention --}}
                <div class="relative pt-1 border-t border-line/50 space-y-1.5">
                    {{-- Preview Balasan Pesan (Bagian 9.1: WhatsApp style reply bar) --}}
                    ...
                    {{-- Dropdown Autocomplete @Mention (Bagian 8.1) --}}
                    ...
                    {{-- Form Input Pesan --}}
                    <form id="course-discuss-form" method="post" action="{{ route('chat.messages.store', $course['id']) }}" class="space-y-1">
                        @csrf
...
<script>
    (function() {
        ...
        const replyAuthorLabel = document.getElementById('reply_author-label');
        ...
        // 2. Pin / Unpin Handling (Bagian 10.2)
        ...
        // 3. Delete Message Handling (Bagian 10.2)
        ...
        // 4. @Mention Dropdown Autocomplete (Bagian 8.1)
        ...
        // 5. Submit Form dengan Enter & AJAX
        if (discussInput && discussForm) {
            ...
            discussForm.addEventListener('submit', async function(e) {
                ...
                try {
                    const payload = {
                        content: content,
                        reply_to_message_id: replyInput.value ? Number(replyInput.value) : null,
                        mentioned_user_ids: selectedMentionUserIds,
                    };

                    const response = await fetch(`/chat/course/${courseId}/messages`, {
                        method: 'POST',
        ...
        // 6. Polling & Real-Time Sync (Bagian 2 & 11)
                ...
                if (chatMessages && data.messages) {
                    ...
                    if (isDifferent) {
                        ...
                        chatMessages.innerHTML = data.messages.map(m => {
                            ...
                            const pinBadge = m.is_pinned ? `<span id="pin-badge-${m.id}" class="inline-flex items-center gap-0.5 text-[9px] font-bold text-amber-700 bg-amber-100/80 px-1 rounded border border-amber-200">📌 Disematkan</span>` : '';
                                    ...
                                    <article class="min-w-0 w-fit max-w-[88%] rounded-xl border p-3 shadow-2xs transition-all ${isMe ? 'bg-[#edf4fb] border-[#cfe0f2]' : 'bg-canvas/70 border-line/60'} relative hover:shadow-xs">
                                        <div class="flex items-center justify-between gap-2">
                                            <div class="flex items-center gap-2 min-w-0">
                                                ...
                                                <div class="min-w-0 flex items-center gap-1.5 flex-wrap">
                                                    <span class="text-xs font-bold text-ink truncate">${m.author}</span>
                                                    ${isMe ? '<span class="text-[10px] font-medium text-brand">(Saya)</span>' : ''}
                                                    ${m.role === 'dosen' ? '<span class="status text-[10px] font-semibold py-0 px-1.5 text-brand bg-brand-soft border border-brand/20">Dosen</span>' : ''}
                                                    ${pinBadge}
                                                </div>
                                            </div>
        ...
        // Real-Time Polling Fallback (interval 3 detik ketika halaman aktif)
        ...
        // Laravel Echo WebSocket Listener (jika Reverb/Echo aktif)
        if (window.Echo) {
            try {
                ...
                console.log('Echo WebSocket fallback ke polling aktif.');
            }
        }
    })();
</script>
