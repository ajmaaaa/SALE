@php
    ...
    $next = $contents->whereIn('type', ['tugas', 'coding', 'kuis', 'uts', 'uas'])->sortBy('due')->first();
    $sks = $course['sks'] ?? '3 SKS';
    $studentsCount = $course['students_count'] ?? (count($course['students'] ?? [1, 2, 3, 4, 5]));
    $assessmentsCount = $course['assessments_count'] ?? $contents->whereIn('type', ['tugas', 'coding', 'kuis', 'uts', 'uas'])->count();
    $type = $course['type'] ?? ($next ? \App\Support\LearningPreview::label($next['type']) : 'Materi kelas');
