...
<div class="space-y-7">
    ...
    <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4" aria-label="Daftar course">
        @forelse ($courses as $course)
            @php
                $currentSec = strtoupper(request('section', 'A'));
                if (!in_array($currentSec, ['A', 'B', 'C'])) $currentSec = 'A';
            ...
            <div class="group flex min-h-64 flex-col overflow-hidden rounded-xl bg-white shadow-sm transition duration-200 hover:-translate-y-0.5 hover:shadow-md border border-line/60">
                @if($course['cover'])
                    <a href="{{ route($routePrefix.'.course.show', ['course' => $course['id'], 'section' => $currentSec]) }}" class="block">
                        ...
                        <div class="px-5 pt-4">
                            ...
                            <h2 class="mt-2 text-lg font-semibold text-ink group-hover:text-brand transition">{{ $course['title'] }}</h2>
                            <p class="mt-1 text-xs text-muted">{{ $course['lecturer'] }}</p>
                        </div>
                    </a>
                @else
                    <a href="{{ route($routePrefix.'.course.show', ['course' => $course['id'], 'section' => $currentSec]) }}" class="relative min-h-32 overflow-hidden bg-brand-dark px-5 py-5 text-white block">
                        @if($course['id'] === 1)
                        ...
                        <div class="relative z-10">
                            ...
                            <h2 class="mt-3 text-xl font-semibold leading-7 text-white group-hover:underline">{{ $course['title'] }}</h2>
                            <p class="mt-1 text-xs text-white/80">{{ $course['lecturer'] }}</p>
                        </div>
                    </a>
                @endif

                <a href="{{ route($routePrefix.'.course.show', ['course' => $course['id'], 'section' => $currentSec]) }}" class="flex flex-1 flex-col px-5 py-4">
