...
class LearningController extends Controller
{
        ...
        if (in_array($data['question_type'], ['pilihan', 'kompleks']) && in_array($data['type'], ['tugas', 'kuis'])) {
            $request->validate(['options' => ['required', function ($attribute, $value, $fail) {
                if (count(array_filter(array_map('trim', explode("\n", $value)))) < 2) {
                    $fail('Masukkan minimal dua pilihan, satu pilihan per baris.');
                }
            }]]);
        }
    ...
    public function discuss(Request $request, int $course, int $item)
    {
        ...
        $messages = session("learning.discussions.$item", []);
        $messages[] = ['author' => 'Ahmad', 'message' => $data['message'], 'time' => now()->format('d M, H:i')];
    ...
    public function submit(Request $request, int $course, int $item)
    {
        ...
        if (! $request->filled('answer') && ! $request->filled('link') && ! $request->hasFile('files') && ! $request->filled('choices')) {
        ...
        $data['files'] = array_map(fn ($file) => $this->upload($file), $request->file('files', []));
